...
def subword_weight_to_word_weight(subword_weight_str, m, smoothing, keep_all_terms):
    ...
    weights = []
    index = 0
    split_item = subword_weight_str.split('\t')

    # random number > max len
    no_G = 99999
...
def tsv_to_weighted_doc(dataset_file_path,
    ...
    dataset = []
    predictions = []
    with open(dataset_file_path) as dataset_file, open(prediction_file_path) as prediction_file, open(output_file_path,
                                                                                                      'w') as output_file:
        n, e, a = 0, 0, 0
...
if __name__ == '__main__':
    ...
    parser.add_argument('prediction_file', help='DeepCT prediction file (test_results.tsv)')
    parser.add_argument('output_file', help='Output File')
    parser.add_argument('m', type=int, help='scaling parameter > 0, recommend 100')
    parser.add_argument('--smoothing', type=str, choices=["none", "sqrt"],
                        help="optionally use sqrt to smooth weights. Paper uses none..")
    parser.add_argument('--keep_all_terms', action='store_true',
                        help="do not allow DeepCT to delete terms. Default: false")
